...
Express + Socket.io server on Replit. UI language is Italian (`locales/it.ts` is the source
of truth; `en` and `sq` follow). A *manche* is one hand; a *partita* is the match they add
up to.
