...
export function verdict(filePath, root) {
...
if (isInvokedDirectly(process.argv[1], import.meta.url)) {
  try {
    ...
    if (process.env.LOOP_TURNS && file) {
      const common = execFileSync("git", ["-C", payload.cwd ?? process.cwd(), "rev-parse", "--path-format=absolute", "--git-common-dir"], {
        encoding: "utf8",
        stdio: ["ignore", "pipe", "ignore"],
      }).trim();
      const reason = verdict(file, path.dirname(common));
      if (reason) {
