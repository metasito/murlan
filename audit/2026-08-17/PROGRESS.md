...
| - [ ] 14 | Carried-forward cleanup | 13 | high | **Opus** | not started | | |
...
| Batch 13 · (new) | CI never runs against merged code on `main` | The run that would test a merge is cancelled by the documentation commit pushed seconds later, and that commit skips every real step because it only touches prose. Batch 11's and batch 12's post-merge runs each executed **0 of 5** real steps, so `main` has shown green having tested nothing. The code is still tested on the PR before merging, so nothing unsafe shipped. Decided: **D12** — `cancel-in-progress` applies to pull requests only. | **Batch 14** | open |
