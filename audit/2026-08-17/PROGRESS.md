...
| Batch 11 · UI-01, UI-02, UI-03, UI-06 | Write the four Playwright viewport assertions their acceptance criteria name (settings scrolled to the delete button at 568pt tall; the last room seat row and invite panel reachable at 390x844; the last home menu row reachable at 375x667; every menu screen ≤ 840pt wide at 1920) | The E2E stack could not boot: `scripts/dev-stack.mjs` hardcodes container `murlan-dev-pg` on port 55432, which the Postgres of the batch that ran beside this one held for its own integration suites. Nothing in the batch is unverified by the unit and native suites — `tests/native/settingsOverlay.test.tsx` and `tests/native/bannerBand.test.tsx` pin the structural half — but the laid-out-box half is asserted, not measured. | **Batch 12** (it already owns the E2E stack repair) | open |
