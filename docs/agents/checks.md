...
- **A spec outside `tests/e2e/` is collected as zero tests, silently** — `testDir` only sees that
  ...
  hand races it for the port; a piped run reports the pipe's exit code, not the suite's — read the
  `N passed / N failed` line, never grep's `$?`.
...
- **Metro keeps one transform cache for the whole machine** (`%TEMP%/metro-cache`), keyed short of
  two real inputs: which checkout (a worktree junction makes one physical file two worktrees at
  equal depth collide over) and the `EXPO_PUBLIC_*` values. `metro.config.js` adds both to
  `cacheVersion`. Both fail silently: the first bundles a route context pointing outside the
  worktree, so `dist/` carries no routes and nothing errors until the browser 404s (#438); the
  second serves one build's inlined constants to another. `scripts/bundleRoutes.mjs` re-checks the
  exported bundle against `app/` on every e2e run. A new worktree's first export, and the first
  after any `EXPO_PUBLIC_*` change, is a cold build (~3 min) — sharing the cache was fast and wrong.
