...
| **The polish pass** |
| Q27 (pass 5) | **UI/UX audit.** ✅ *A durable check for the bug class pass 4 found by hand*: `tests/e2e/tapTargets.spec.ts` sweeps every control on the menus, the online lobby and the game table at three viewport sizes and asserts none is covered at its own centre by an inert element. Narrow on purpose — controls legitimately overlap each other (the hand fans its cards), and a screen deliberately covering another is not a defect; what is never legitimate is a button buried under a label or a divider. **Verified able to fail** by reintroducing the flex bug: it reports `Crea Stanza blocked by DIV:oppure` exactly. Building it surfaced two of my own false positives, both fixed in the probe: intentional full-screen overlays (the tutorial over home, the rotate prompt over the landscape-locked table), and controls merely *scrolled out of* a list, whose rect still sits inside the window. The second cost two wrong fixes to `app/lobby.tsx`, both reverted — the lobby was correct all along, twice. **Also worth recording:** a `git stash push` of an already-committed fix stashes nothing, so a "verified able to fail" run built the fixed code and passed vacuously. Reverting the change for real is the only honest way to test a new check. **Still open:** the online *room* and *replay/leaderboard* screens are not yet in the sweep. | XL | owner |
