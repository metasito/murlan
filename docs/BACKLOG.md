...
| O13 | `context/SocketContext.tsx` is the other, and it has a live bug underneath | An owner ruling on the same pattern, plus the fix already queued as its own task. **Measured:** the same probe → `BAILS — line 53 Cannot access refs during render`, twice. **Unlike O12 this one is not cosmetic:** `contextValue`'s `useMemo` at :287 reads `socket: socketRef.current` with no socket in its dependency list (`[connected, onlineIds, pendingInvite, gameInvites]`), so between the socket being created and the `connect` event landing, every consumer of `useSocket()` reads a stale `null`. Fixing the bailout and fixing that are the same edit, so do them together rather than separately. |
