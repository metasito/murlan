...
| **The polish pass** |
| Q27 (pass 1) | **UI/UX audit.** First pass done on the menu surfaces at 390×844, 844×390 and 1280×720, driven through the real web build and inspected as screenshots. **No layout defect confirmed** — the lobby is sound in both orientations, the personality and cosmetics pickers render correctly, and the settings modal's swatch rows fit at every size. **The method finding is the useful output:** a screenshot of a scrollable region shows only its top, and four apparent defects (the 4th seat "occluded" by the start bar, the format section "clipped", the landscape columns "broken") all evaporated once each scroll area was actually scrolled. Landscape splits into two independently scrolling columns, so each needs its own wheel position. Any future pass must scroll every scrollable region before recording anything, or it generates false positives at a high rate — this one generated four out of four. **Still open:** the game table itself (the capture timed out before reaching it) and the online surfaces (need auth). **One unconfirmed observation** to check against the code, not a screenshot: on the home screen the primary "Offline" button's gold fill appears inset and square-cornered inside its rounded, bordered container, unlike the other rows. | XL | owner |
