...
| **The polish pass** |
| Q27 (pass 3) | **UI/UX audit — online surfaces.** ✅ *Three real defects in the room screen*, all from one botched edit in 471b7ea (pre-dating this session): `{formatControls}` rendered **twice** in landscape; **never rendered at all in portrait**, so an online host on a phone in portrait had no way to choose between a full match and a single manche; and the left column had no scroll container, so its content ran underneath the start button instead of being reachable. All three fixed and verified by screenshot. **Verified working:** the ladder screen (rating, derived season `2026-08`, the provisional gate, the empty state), the profile's cards, and the room's bot-personality picker. **Probable defect, not yet confirmed:** on the *online lobby* at 390×844 the "Crea Stanza" button appears to overlap the section beneath it, and Playwright's click on it timed out in two independent runs while the button was demonstrably enabled — a click failing that way is what an overlapping element causes. If real it means a room cannot be created on a phone in portrait. Next pass confirms and fixes. **Still open:** the game table at more sizes. | XL | owner |
