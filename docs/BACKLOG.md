...
| Q10b | **The two-joker exception can strand the player on the result screen.** Caught by a full E2E sweep (`offline.spec.ts`, "a match plays multiple hands"), on the branch that only fires when the loser holds both Jokers — which is why it has gone unseen. After "OK, inizia!" the URL becomes `/game` and `waitForURL` resolves, but `[data-testid="game-table"]` is never found *at all*, and the screenshot shows the result screen still mounted with the overlay still up, faded mid-transition. `gameState` is not null (the game screen would bounce to `/` — it does not), so the table should have rendered. Reads like a replace that the navigator never completed; compare the known `REPLACE navigation action not handled` pitfall in `CLAUDE.md`. Note the overlay's own 2500ms auto-navigate timer and the button both call `router.replace("/game")`, and nothing ever clears `bothJokersException`. **Reproducing needs the branch**, so a deterministic repro wants the state driven directly (as `tests/native/resultExchange.test.tsx` does) rather than waiting for the deal. Evidence: `tests/e2e/test-results/offline-offline-vs-AI-*/test-failed-1.png`. **Ruled out so far:** it is not a double-mount — `app/result.tsx` returns early for landscape, so only one overlay renders; and it is not `gameState` going null, which would bounce to `/`. **Blocked on a harness gap:** the obvious next step is asserting the overlay navigates exactly once, but `expo-router` cannot currently be mocked or spied in the jest-expo suite — jest-expo supplies its own module and a local `jest.mock` does not take. Solving that is a prerequisite for a deterministic repro at the render layer, and is worth doing on its own: nothing can currently assert navigation anywhere. | M | owner |
