...
| Q9 (1/3) | **E2E coverage gaps.** ✅ *The result screen's rematch exchange.* Not covered by Playwright, and it cannot be: the interactive branch only appears when the human won the previous hand, so an E2E assertion on it is a coin flip. `CardExchangeOverlay` moved out of `app/result.tsx` into `components/ResultExchangeOverlay.tsx` and is now driven directly by `tests/native/resultExchange.test.tsx` — every branch reachable, including the two-joker exception and the documented no-legal-giveback fallback. **Still open:** teams-mode partner coordination, and the AFK auto-pass / disconnect-reconnect paths (the server half of the latter is already covered by "a vacated seat does not deadlock the table"; what is missing is the client's presentation). | M | A21 |
