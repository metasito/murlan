...
| N10 | **Every confirmation and error dialog is a no-op on web.** `react-native-web` implements `Alert` as `class Alert { static alert() {} }` — an empty function. The app imports `Alert` straight from `react-native` in twelve places with no shim, and web is what Replit serves. So on the platform the game is actually played on today: **you cannot quit an offline or online game** (the confirm never appears, so `resetGame()` and the navigation never run), you cannot leave a room, **errors on the online lobby and room are invisible**, friend-request results are invisible, and the delete-account confirmation never opens. Found behaviourally first — an E2E test quit a game, reloaded, and the interrupted-match offer was still there — then confirmed in `node_modules/react-native-web/dist/exports/Alert/index.js`. The fix is one cross-platform confirm/alert used everywhere, not twelve `Platform.OS` branches; it needs a design because it touches every screen and because a `window.confirm` is not what the native side should get. Add the quit half of `tests/e2e/offlineResume.spec.ts` back once it lands. | M | measured |
...
| O11 | Q11 and Q12 cannot progress: both need one green CI run, and CI needs a push | A push. `origin/main` is 74 commits behind local `main`, so `.github/workflows/maestro.yml` does not exist on the remote at all — `workflow_dispatch` has nothing to dispatch. Everything else about the job is ready and `gh` is authenticated here; only the decision to publish the branch is missing. Once one run is green, promoting the trigger to `on: push` is one line, and the exchange/rematch flows can be written against a runner that has been proven to hold the animated table. |
