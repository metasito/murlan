...
| Q13 | **The sound effects sound bad.** They are synthesized by `scripts/gen-sounds.js` and it shows. Either source good CC0/CC-BY audio or find a materially better synthesis approach. Twelve effects in `assets/sounds/`. Whatever replaces them must stay small (the set is 257 KB today, already down 53%) and keep the same file names so `lib/sounds.ts` is untouched. | M | owner |
...
| Q15 | **Colourblind-safe suit differentiation.** Improves legibility for everyone at small card sizes, not only for colourblind players. Verify what the current palette actually does before changing it. | S | BRIEF T3 |
