...
| N1 | **`room:unspectate` is the only inbound event that skips the safety wrapper.** Every other one of the sixteen goes through `onEvent`, which validates its payload, rate-limits it and — the part that matters — isolates a throw so one bad emit cannot take down every table on the server. This one is a raw `socket.on`. It carries no payload, so validation is moot, but the error isolation and the rate limit are not, and the exception is invisible: `server/socketSchemas.ts` still says it holds schemas "for every inbound socket event". | S | code read |
| N2 | **The ladder shows its season as `2026-08`.** A sortable machine key, in front of a player, where a month name belongs. The key itself must not change — it is half the `user_ratings` primary key — so this is a presentation-layer mapping only. | S | code read |
| N3 | **The home screen defines its own `MenuButton`.** `app/index.tsx:43` shadows `components/MenuButton.tsx`, which twelve other screens import, against the design-system rule in `CLAUDE.md`. They are genuinely different things — a navigation row with a chevron and a staggered entrance versus a pill CTA — so the fix is to name the local one for what it is, not to force one into the other. The duplication is not theoretical: the accent-padding bug lived in the local copy precisely because the shared one clips its gradient and has no padding conflict. | S | Q27 pass 2 |
| N4 | **`active_games` has no retention for rooms nobody comes back to.** A row deliberately survives a server restart so a match in progress can be rehydrated, and `disposeGame` removes it on every ordinary ending. What removes the row for a room that was mid-game when the server went down and that nobody ever rejoins? Nothing found. `match_replays` got an age-based prune for the same reason; this wants the same look. Confirm before building — the disconnect grace path may already cover more of this than it appears to. | S | code read |
| N5 | **An offline game is lost the moment the app goes away.** `context/GameContext.tsx` holds the whole match in memory and writes nothing; `docs/ARCHITECTURE.md` §2 says so outright. Online games survive a restart through `active_games`, so the weaker guarantee is on the mode played without a network — on a phone, where a call or a background kill mid-hand is ordinary. AsyncStorage is already a dependency and already carries settings and locale, so this needs no new storage and no server. The interesting part is what to persist: the engine state alone is not enough, since the match scoreboard and the rematch answers live beside it. | M | code read |
| N6 | **The aesthetics half of the UI audit.** Q27's sweep proved every control is reachable at three sizes; it says nothing about whether the game *reads* well — motion that overshoots or jitters, spacing rhythm, whether the table is legible at a glance, whether the hand is comfortable at fourteen cards on a small phone. That needs judgement against how shipped card games feel, not a DOM probe, and it is the half Q27 originally asked for. | L | Q27 |
| **The polish pass** |
