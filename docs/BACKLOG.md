...
| **The polish pass** |
| Q27 (pass 4) | **UI/UX audit — online surfaces.** ✅ *A room could not be created on a phone in portrait.* `CreateSection` and `JoinSection` each carry `flex: 1` for the landscape row where they sit side by side; stacked in the portrait `ScrollView` that made them fight over the container's height instead of sizing to content, so they overlapped and the "oppure" divider came to rest on top of the create button and swallowed its taps. Nothing looked wrong — the button was visible, enabled and correctly labelled — it simply could not be pressed. Confirmed with `elementFromPoint`: the hit target was the divider, not the button, and the click timed out at 8s; after the fix the hit target is the button's own label and the click lands in 89ms. The flex is now landscape-only, and `tests/e2e/online.spec.ts` gains a portrait create-room test, verified able to fail. ✅ *Three room-screen defects* from a botched edit in 471b7ea: the format picker rendered twice in landscape, never in portrait, and the left column overflowed under the start button. **Verified working:** the ladder screen, the profile cards, the bot-personality picker. **Method:** four passes in, the rule that pays is *confirm in code or with a DOM probe before calling anything a defect* — pass 1 gave four false positives from screenshots alone; passes 3 and 4 gave four real bugs, each pinned by something other than a picture. **Still open:** the game table at more sizes. | XL | owner |
