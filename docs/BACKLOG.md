...
| **The polish pass** |
| Q27 (pass 2) | **UI/UX audit.** ✅ *One real defect found and fixed*: the home screen's accent button set `padding: 0` to let its gradient reach the edges, but the base style sets `paddingVertical`/`paddingHorizontal`, and React Native resolves the longhand over the shorthand whatever the order — so the padding stayed and the gold sat inset inside the border with square corners against a rounded container. Zeroed as longhands; verified before and after. **Method, confirmed twice over:** pass 1 produced four false positives from screenshots of scrollable regions; pass 2 produced a fifth (two adjacent "14"s on the table turned out to be the turn timer and the card count coinciding). Every visual suspicion is now checked against the code before it is called a defect — which is how the accent bug was actually pinned. **Recorded, not yet acted on:** `app/index.tsx` defines its own local `MenuButton` instead of using `components/MenuButton.tsx`, against the design-system rule. That duplication is exactly why this bug existed in one button and not the other — the shared component clips its gradient with `overflow: hidden` and has no padding conflict. Collapsing the two is its own item. Also noted: the turn timer and the card badge sit adjacent at top-right with no visual distinction, so two unrelated numbers read as one — a design call, not a defect. **Still open:** the online surfaces (need auth), and the table at more sizes than 844×390. | XL | owner |
