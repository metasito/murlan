...
| **The deal at 2 players** | **Deal 21 each (42 of 54); the other 12 stay face down and unused.** | Dealing the whole deck at 2 players lets each player deduce the other's exact hand by elimination — Pagat's own diagnosis of this exact family of games. 21 each (docs/research/card-dealing-variable-player-count.md §10) keeps real concealment (12 hidden cards) while a smaller 13/14-card hand would strip the four-of-a-kind bomb almost to nothing (~74% of deals losing a Joker, vs. ~40% at 21 each). Reinstates the "lowest dealt card opens" fallback at 2 players only — 3p and 4p keep the fake-fallback-free full deal above. |
