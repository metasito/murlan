...
This was reproduced directly, and the reproduction is now a suite CI runs
(`tests/integration/crossInstance.test.ts`): two instances against one database silently split
broadcasts, and Cloud Run's own configuration states "it is the running configuration," not a
risk to check. Scaling was never deferred — it was already the deployed shape before anyone
decided it.
