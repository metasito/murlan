...
- **Ambiguity** — the implementation deals the other way round. `dealCards` gives successive cards to seat `start + i` while play runs the other direction (`getNextActivePlayer`), so the deal is anticlockwise against a clockwise game. Resolved by the decision recorded in `docs/BRIEF.md` §3.1: **the anticlockwise deal stands.** It decides only which seats get the 14th card of a 54-card deal, and the rotating deal start (§3.1) already moves that round the table every manche.
...
4. **Royal straight / flush:** Tier 2 implementations (murlanarena, murlan.app) add a "Flush" = 5+ consecutive cards of one suit, and rank it **above bombs**: *"Quadruplets beat regular combinations but only lose to higher Quadruplets or Flushes… Flush is the strongest combination type as it can beat anything."* **The Albanian/Italian Tier-1 rules do not contain this combination at all.** This app follows Tier 2 here as a **resolved house rule for this implementation** (not an open Ambiguity), per the decision recorded in `docs/BRIEF.md` §3.1: a royal straight beats a bomb of any strength, and a bomb can never beat a royal straight — see §7.3.
