...
written at all, only one of three fixed words of our own saying which part of the service was
...
never your email address and never the contents of a crash report — a crash report is kept in the
...
| Server logs — the line written for each request | Nothing in it identifies you: no IP address, no headers, no query string, and no id from the address, which is written only when it matches one of our own routes and then only in that route's general form; when it matches none, one of three fixed words of ours saying which part of the service was asked for |
| Server logs — every other line (refused game messages, account actions) | Our hosting provider's retention; not removed by account deletion. Carries your account number only — no email address, and no crash report |
