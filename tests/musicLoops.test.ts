// tests/musicLoops.test.ts — the four music loops still join seamlessly.
//
...
//
// Decoded through Chromium's OfflineAudioContext via Playwright — the same
// route scripts/build-sounds.mjs uses, since there is no ffmpeg here.
...
/** What lib/music.ts requires. A file added here without a require is dead weight. */
...
describe("the music loops join seamlessly", () => {
  for (const track of TRACKS) {
    test(`${track} wraps without a step`, () => {
    ...
    test(`${track} has no silence at either end`, () => {
      const m = measured[track];
      // The other way a loop fails. Opus carries encoder delay as pre-skip, and
      // a container that does not declare it leaves that delay in the buffer as
      // a gap — the reason #121 chose WebM over Ogg.
      assert.ok(m.headMs < 1, `${track} starts with ${m.headMs.toFixed(1)} ms of silence`);
      assert.ok(m.tailMs < 1, `${track} ends with ${m.tailMs.toFixed(1)} ms of silence`);
    });
  }

  test("all four are 48 kHz stereo, as #96 settled", () => {
    for (const track of TRACKS) {
      assert.equal(measured[track].sampleRate, 48000, `${track} is not 48 kHz`);
      assert.equal(measured[track].channels, 2, `${track} is not stereo`);
    }
  });

  // The floor. Every assertion above is satisfied by a file of pure silence:
  // its seam is zero and its rate is whatever was asked for.
...
test("assets/music holds exactly the tracks lib/music.ts requires", () => {
  const onDisk = readdirSync(musicDir)
  ...
  for (const track of TRACKS) {
    assert.ok(
      source.includes(`assets/music/${track}.webm`),
      `lib/music.ts never requires ${track}.webm, so Metro will not bundle it`
    );
  }
});
