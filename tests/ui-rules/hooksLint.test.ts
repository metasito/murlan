// tests/ui-rules/hooksLint.test.ts — the three eslint-plugin-react-hooks 7 rules #891
...
// own list, and none of these three are on it — `tests/ui-rules/reactCompiler.test.ts`
...
/** The offender text, once, so the cases below can assert against it rather than restate it. */
 ...
 *
 * Only the three rules `ADOPTED` names, where `tests/ui-rules/reactCompiler.test.ts`
...
describe("the react-hooks 7 rules #891 adopted stay adopted", () => {
  test("the reason a suppression is refused is the ESLint one", () => {
    // The compiler charges nothing for these three — `tests/ui-rules/reactCompiler.test.ts`
  ...
  test("there is no suppression of an adopted rule anywhere the rule is on", () => {
    ...
    assert.deepEqual(
      ...
      "which of these three is on is eslint.config.js's to say, so the effect is rewritten " +
        ...
        "tests/ui-rules/reactCompiler.test.ts measures which rules it does charge for"
    );
  });
});
