// tests/ui-rules/reactCompiler.test.ts — every screen and component stays under the
// React Compiler.
//
...
test("a suppression of a rule #891 adopted costs the compiler nothing", () => {
  ...
  assert.deepEqual(
    ...
    [],
    "the compiler now charges for one of the three, so the reason tests/ui-rules/hooksLint.test.ts and " +
