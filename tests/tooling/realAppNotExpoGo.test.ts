// tests/tooling/realAppNotExpoGo.test.ts — Expo Go cannot host a UI suite, and the
...
function landscapeBlock(rel: string): string {
  const blocks = flowBody(rel, code).split(/^(?=- )/m);
  const found = blocks.filter((b) => /setOrientation: LANDSCAPE_(LEFT|RIGHT)/.test(b));
  assert.equal(found.length, 1, `${rel} should rotate to landscape from exactly one block`);
  return found[0];
}
