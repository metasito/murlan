...
//
// Structural, like tests/ui-rules/orientation.test.ts and tests/ui-rules/tokenRoles.test.ts: the
...
test("the events that exist are registered through the wrapper", () => {
  ...
  assert.deepEqual(
    ...
    `every onEvent registration belongs to the socket family (${SOCKET_FAMILY.join(", ")}), ` +
      `where this file and tests/server/serverLoadable.test.ts look for it. Splitting another ` +
