...
test("the server's sockets are typed with the protocol's event maps", () => {
  const untyped = readdirSync(path.join(root, "server"))
    .filter((f) => f.endsWith(".ts") && f !== "socketTypes.ts")
    .filter((f) => /import\s+(?:type\s+)?\{[^}]*\b(?:Server|Socket)\b[^}]*\}\s+from\s+"socket\.io"/.test(read(`server/${f}`)))
    .filter((f) => !(f === "socket.ts" && /new Server<ClientToServerEvents, ServerToClientEvents>/.test(read("server/socket.ts"))));
  assert.deepEqual(untyped, [], "import SocketServer/GameSocket from server/socketTypes.ts instead");
});
