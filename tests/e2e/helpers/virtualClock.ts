// One virtual clock for both sides of tests/e2e/mockupParity.spec.ts (#1255). The measurements
 ...
 * two runs then differ (the mockup's lamp sway moved up to 11 px).
 */
...
/**
 * After navigation. `page.clock` drives rAF and timers but not CSS animations; CDP puts those on
 * the same clock. Then the clock onto rAF's grid: rAF fires at `16 - ticks % 16`, and the real
