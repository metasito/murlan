...
`node tools/loop/next-ticket.mjs`. Take the issue it prints only if the route is `triage` — the
...
- **Remove `needs-triage` and then `in-progress`.** Triage never closes anything on its own — a
  `rejected` issue stays open (`docs/BRIEF.md`) — and `classify()` in `tools/loop/next-ticket.mjs`
