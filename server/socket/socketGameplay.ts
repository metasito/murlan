// server/socket/socketGameplay.ts — everything a player does at a live table: playing
...
import { socketRoomMap } from "../game/gameRoom.ts";
  ...
  userId,
    ...
    onEvent(
      ...
      async ({ roomId }) => {
        ...
        try {
          outcome = await applyOrForward(io, { kind: "rejoin", roomId, userId });
        } catch (err) {
