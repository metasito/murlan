...
// `broadcastGameState`/`persistGameState` → `gameTurn.ts` `armTurn`. A play
// that empties a hand takes a sixth file instead of `armTurn`: `gameOver.ts`
// `handleGameOver`; a pass never reaches it.
//
...
// expiry, calls `gameTurn.ts` `vacateSeat` — settling the departure itself,
// through a bot takeover, `gameOver.ts` `voidAbandonedMatch`, or
// `gameTurn.ts`'s own `concedeHand` into `gameOver.ts` `handleGameOver` —
// then re-checks the rematch gate through `dealIfSeatLeftGateClosed` here.
// Between hands it instead arms a separate, shorter lobby grace and never
// reaches `vacateSeat`.
//
 ...
 * vote on this table's roster is counted in, so a second vote reuses this
 ...
 */
async function endMatchVoteAction(
  io: SocketServer,
  game: OnlineGameState,
...
function reclaimableSeat(game: OnlineGameState, userId: string): number | null {
  for (const [seat, vacated] of game.vacatedSeats) {
    if (vacated.userId === userId) return seat;
  }
  return null;
}
...
async function rejoinAction(
  ...
  if (seat === null) {
    ...
    reclaimSeat(game, reclaim, userId);
    seat = reclaim;
    // Every viewer's own `vacated` flag for this seat must flip false, and
    // the returning player's hand — still theirs; `vacateSeat` never touched
...
async function applyTableAction(
  ...
  switch (action.kind) {
    ...
    case "endMatchVote":
      return endMatchVoteAction(io, game, action);
    case "rejoin":
