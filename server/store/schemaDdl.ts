 ...
 * `drizzle.config.ts` excludes it from push, and `server/http/session.ts` runs
  ...
  if (col.default === undefined) {
    throw new Error(
      `schemaStatements: column "${tableName}.${col.name}" has a client-side ` +
        ...
        `expressed as DDL — update schemaStatements() in server/store/schemaDdl.ts.`
    );
  }
        ...
        if (!is(chunk, StringChunk)) {
          throw new Error(
            `schemaStatements: column "${tableName}.${col.name}" has a ` +
              ...
              `schemaStatements() in server/store/schemaDdl.ts.`
          );
        }
    ...
    if (ref.columns.length !== 1 || !column || !foreignColumn) {
      throw new Error(
        `schemaStatements: table "${cfg.name}" has a composite foreign key, ` +
          `which is not supported — update schemaStatements() in server/store/schemaDdl.ts.`
      );
    }
    ...
    if (cfg.checks.length > 0) {
      throw new Error(
        `schemaStatements: table "${cfg.name}" has CHECK constraints, which ` +
          `are not supported — update schemaStatements() in server/store/schemaDdl.ts.`
      );
    }
      ...
      if (!values) {
        throw new Error(
          `schemaStatements: enum column "${cfg.name}.${col.name}" has no ` +
            `enumValues — update schemaStatements() in server/store/schemaDdl.ts.`
        );
      }
    ...
    if (stack.has(entry.cfg.name)) {
      throw new Error(
        `schemaStatements: circular foreign key dependency involving table ` +
          `"${entry.cfg.name}" — update schemaStatements() in server/store/schemaDdl.ts.`
      );
    }
      ...
      if (!indexName) {
        throw new Error(
          `schemaStatements: table "${cfg.name}" has an unnamed index — ` +
            `update schemaStatements() in server/store/schemaDdl.ts.`
        );
      }
    ...
    if (!declaredDedupes.has(indexName)) {
      throw new Error(
        `schemaStatements: DEDUPE_ON_BOOT names "${indexName}", which no table ` +
          `in shared/schema.ts declares — update server/store/schemaDdl.ts.`
      );
    }
  }
